...
class Parameters:
    ...
    def __init__(
        ...
        if not self.helpRequested:
            ...
            self.pickles = {
                x: os.path.join(workdir, y) for x, y in self.__PICKLE_FNS.items()
            }
    ...
    def __checkInstallation(self) -> None:
        ...
        if vers[0] < SCI_VER[0] or (vers[0] >= SCI_VER[0] and vers[1] < SCI_VER[1]):
            raise BaseException(
                f"'scipy'{BAD_VER}{'.'.join(map(str,SCI_VER))} or above)"
            )
    ...
    def __parseArgs(self) -> None:
        ...
        ERR_MSG_3 = (
            "must specify exactly two integers for bad outgroup PCR product length"
        )
        ...
        def printHelp(advanced: bool):
            ...
            DEF_OPEN = " (default: "
            CLOSE = ")"
            WIDTH = max(map(lambda x: len("--" + x.replace("=", "")), LONG_OPTS)) + len(
                GAP
            )  # argument list determines column width

            # primary help message
            HELP_MSG = (
                f"{EOL}Finds pairs of primers suitable for a group of input genomes{EOL}"
        ...
        # give help if requested
            ...
            HELP_FLAGS[0] in sys.argv or HELP_FLAGS[1] in sys.argv or len(sys.argv) == 1
        ):
            self.helpRequested = True
            printHelp(False)
    ...
    def __getIntermediateDirname(self, makeWd: bool) -> str:
        ...
        allParamFns = glob.glob(
            os.path.join(
                os.curdir,
                f"{Parameters._WORKDIR_PREFIX}*",
                Parameters.__PICKLE_FNS[Parameters._PARAMS],
            )
        )
