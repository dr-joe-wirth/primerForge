...
def __runOnePcr(
    allContigsFna: str, query: str, minGood: int, minPerfect: int, tileSize: int
...
def __runAllPcrs(
    params: Parameters, pairs: list[tuple[Primer, Primer]]
) -> dict[tuple[str, str], set[tuple[str, int]]]:
    ...
    def genArgs() -> Generator[tuple[str, str], None, None]:
        ...
        while start <= numPairs:
            ...
            yield (
                params.allContigsFna,
                query,
                params.isPcr_minGood,
                params.isPcr_minPerfect,
                params.isPcr_tileSize,
            )
...
def __filterPairs(
    ...
    pcrs: dict[tuple[str, str], list[tuple[str, int]]],
) -> None:
    ...
    for pair, observed in pcrs.items():
        ...
        expected = {
            (x.contig, x.size) for x in pairs[pair].values() if x.contig != "NA"
        }
