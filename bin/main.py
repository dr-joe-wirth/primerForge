...
def __getCandidates(
    params: Parameters, sharedExists: bool, clock: Clock
...
def __getUnfilteredPairs(
    params: Parameters, candidateKmers: dict[str, dict[str, list[Primer]]], clock: Clock
    ...
    params.dumpObj(
        pairs, params.pickles[Parameters._PAIR_1], "unfiltered pairs", prefix=" " * 4
    )

    return pairs


def __removeOutgroup(
    params: Parameters, pairs: dict[tuple[Primer, Primer], dict[str, Product]]
) -> None:
    ...
    if params.outgroupFns != []:
        ...
        params.log.info(
            f"{GAP}{MSG_1A}{startingNum - finalNum}{MSG_1B}{finalNum}{MSG_1C}"
        )

        # dump the results
        params.dumpObj(
            pairs, params.pickles[Parameters._PAIR_2], "filtered pairs", prefix=GAP
        )
...
def __validatePairs(
    ...
    # dump the results
    params.dumpObj(
        pairs, params.pickles[Parameters._PAIR_3], "validated pairs", prefix=GAP
    )


def __generateBedRows(
    num: int,
    primer: Primer,
    ...
    for name in ingroup:
        ...
        yield SEP.join(
            (products[name].contig, str(start), str(end), str(primer), str(num), strand)
        ) + "\n"
...
def __writeBedFile(
    ...
    sortOrder: list[tuple[Primer, Primer]],
    ingroup: set[str],
) -> None:
...
def __writePrimerPairs(
    ...
    def getHeaders(names) -> list[str]:
        # constants
        HEADERS = ("fwd_seq", "fwd_Tm", "fwd_GC", "rev_seq", "rev_Tm", "rev_GC")
        CONTIG = "_contig"
        LENGTH = "_length"
    ...
    clock.printDone()
    params.log.info(
        f"{MSG_2A}{len(pairs)}{MSG_2B}'{os.path.relpath(params.resultsFn)}'"
    )
    clock.printStart(
        f"{MSG_2A}{len(pairs)}{MSG_2B}'{os.path.relpath(params.resultsFn)}'"
    )
