...
class ResultsTest(unittest.TestCase):
    ...
    DEFAULT_INGROUP = {
        INGROUP_FILES[
            0
        ]: "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/003/034/305/GCF_003034305.1_ASM303430v1/GCF_003034305.1_ASM303430v1_genomic.gbff.gz",
        INGROUP_FILES[
            1
        ]: "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/003/034/275/GCF_003034275.1_ASM303427v1/GCF_003034275.1_ASM303427v1_genomic.gbff.gz",
        INGROUP_FILES[
            2
        ]: "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/003/034/345/GCF_003034345.1_ASM303434v1/GCF_003034345.1_ASM303434v1_genomic.gbff.gz",
    }
    DEFAULT_OUTGROUP = {
        OUTGROUP_FILES[
            0
        ]: "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/900/489/555/GCF_900489555.1_MMC68/GCF_900489555.1_MMC68_genomic.gbff.gz",
        OUTGROUP_FILES[
            1
        ]: "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/018/389/745/GCF_018389745.1_ASM1838974v1/GCF_018389745.1_ASM1838974v1_genomic.gbff.gz",
    }
    ...
    ALTRNTE_INGROUP = {
        INGROUP_FILES[
            0
        ]: "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/001/650/295/GCF_001650295.1_ASM165029v1/GCF_001650295.1_ASM165029v1_genomic.gbff.gz",
        INGROUP_FILES[
            1
        ]: "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/008/727/175/GCF_008727175.1_ASM872717v1/GCF_008727175.1_ASM872717v1_genomic.gbff.gz",
        INGROUP_FILES[
            2
        ]: "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/002/208/865/GCF_002208865.2_ASM220886v2/GCF_002208865.2_ASM220886v2_genomic.gbff.gz",
    }
    ALTRNTE_OUTGROUP = {
        OUTGROUP_FILES[
            0
        ...
        OUTGROUP_FILES[
            1
        ]: "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/000/006/945/GCF_000006945.2_ASM694v2/GCF_000006945.2_ASM694v2_genomic.gbff.gz",
    }
    ...
    def setUpClass(cls) -> None:
        ...
        cls.seqFiles: list[str] = list(
            map(
                lambda x: os.path.join(ResultsTest.TEST_DIR, x),
        ...
        if not os.path.exists(ResultsTest.params.resultsFn) or not os.path.exists(
            ResultsTest.params.bedFn
        ):
            # run primerForge
        ...
        cls.results: dict[tuple[Seq, Seq], Result] = ResultsTest._parseResultsFile(
            ResultsTest.params.resultsFn
        )
        clock.printDone()

        # load the bed file into memory
        clock.printStart("reading BED file into memory")
        cls.bed: dict[tuple[Seq, int], dict[str, tuple[int, int, str]]] = (
            ResultsTest._parseBedFile(ResultsTest.params.bedFn)
        )
        clock.printDone()

        # load the genomic sequences into memory
        clock.printStart("reading genomic sequences into memory")
        cls.sequences: dict[str, dict[str, dict[str, Seq]]] = (
            ResultsTest._loadGenomeSequences()
        )
        clock.printDone()
        ...
        clock.printStart(
            "getting binding sites for all kmers and primers", end="...\n", spin=False
        )
        cls.bindingSites: dict[Seq, dict[str, dict[str, dict[str, list[int]]]]] = (
            ResultsTest._getKmerBindingSites(
                cls.results, cls.sequences, cls.params.minLen, cls.params.maxLen
            )
        )
        clock.printDone()
    ...
    def _getParameters(numThreads: int) -> Parameters:
        ...
        existing = glob.glob(
            os.path.join(ResultsTest.TEST_DIR, Parameters._WORKDIR_PREFIX + "*")
        )
        if existing != []:
        ...
        for key in params.pickles.keys():
            params.pickles[key] = os.path.join(
                newDir, os.path.basename(params.pickles[key])
            )

        # update the location of the all_contigs.fna file
        params.allContigsFna = os.path.join(
            newDir, os.path.basename(params.allContigsFna)
        )
                ...
                # extract data if not a  header
                    ...
                    for idx in key.keys():
                        ...
                        if LEN_SUFFIX == key[idx][-len(LEN_SUFFIX) :]:
                            ...
                            additional[name][ResultsTest.PCRLEN] = list(
                                map(int, row[idx].split(SEP_2))
                            )
    ...
    def _parseBedFile(
        fn: str,
    ) -> dict[tuple[Seq, int], dict[str, tuple[int, int, str]]]:
    ...
    def _loadGenomeSequences() -> dict[str, dict[str, dict[str, Seq]]]:
        ...
        """
        return {
            os.path.basename(fn): ResultsTest._loadOneGenomeSequence(fn)
            for fn in ResultsTest.seqFiles
        }
    ...
    @staticmethod
    def _getKmersForOneContig(
        seq: dict[str, Seq], minLen: int, maxLen: int, primers: set[str]
    ) -> dict[str, dict[Seq, list[int]]]:
        ...
        # initialize output
        out = {Primer.PLUS: dict(), Primer.MINUS: dict()}
        ...
        for k in range(minLen, maxLen + 1):
            ...
            fmers = [
                (kmer, start) for start, kmer in getAllKmers(fwd) if kmer in primers
            ]
            rmers = [
                (kmer, length - start - 1)
                for start, kmer in getAllKmers(rev)
                if kmer in primers
            ]
    ...
    @staticmethod
    def _getKmerBindingSites(
        results: dict[tuple[Seq, Seq], Result],
        sequences: dict[str, dict[str, dict[str, Seq]]],
        minLen: int,
        maxLen: int,
    ) -> dict[Seq, dict[str, dict[str, dict[str, list[int]]]]]:
        ...
        for name in sequences.keys():
            ...
            for contig in sequences[name].keys():
                ...
                kmers = ResultsTest._getKmersForOneContig(
                    sequences[name][contig], minLen, maxLen, allPrimers
                )
                ...
                for kmer in allPrimers:
                    ...
                    bindingSites[kmer][name][contig] = bindingSites[kmer][name].get(
                        contig, dict()
                    )
                    bindingSites[kmer][name][contig][Primer.PLUS] = bindingSites[kmer][
                        name
                    ][contig].get(Primer.PLUS, list())
                    bindingSites[kmer][name][contig][Primer.MINUS] = bindingSites[kmer][
                        name
                    ][contig].get(Primer.MINUS, list())
    ...
    def testB_checkTm(self) -> None:
        ...
        for fwd, rev in self.results.keys():
            self.assertEqual(
                self.results[(fwd, rev)].fwdTm,
                round(primer3.calc_tm(str(fwd)), 1),
                f"{FAIL_MSG}{fwd}",
            )
            self.assertEqual(
                self.results[(fwd, rev)].revTm,
                round(primer3.calc_tm(str(rev)), 1),
                f"{FAIL_MSG}{rev}",
            )
    ...
    def testC_tmWithinRange(self) -> None:
        ...
        for fwd, rev in self.results.keys():
            self.assertGreaterEqual(
                self.results[(fwd, rev)].fwdTm, self.params.minTm, f"{FAIL_MSG}{fwd}"
            )
            self.assertGreaterEqual(
                self.results[(fwd, rev)].revTm, self.params.minTm, f"{FAIL_MSG}{rev}"
            )
            self.assertLessEqual(
                self.results[(fwd, rev)].fwdTm, self.params.maxTm, f"{FAIL_MSG}{fwd}"
            )
            self.assertLessEqual(
                self.results[(fwd, rev)].revTm, self.params.maxTm, f"{FAIL_MSG}{rev}"
            )
    ...
    def testD_tmDiffWithinRange(self) -> None:
        ...
        for fwd, rev in self.results.keys():
            tmDiff = abs(
                self.results[(fwd, rev)].fwdTm - self.results[(fwd, rev)].revTm
            )
            self.assertLessEqual(
                tmDiff, self.params.maxTmDiff, f"tm diff to large for {fwd}, {rev}"
            )
    ...
    def testE_checkGcPercent(self) -> None:
        ...
        for fwd, rev in self.results.keys():
            self.assertEqual(
                self.results[(fwd, rev)].fwdGc,
                round(ResultsTest._getGc(fwd), 1),
                f"{FAIL_MSG}{fwd}",
            )
            self.assertEqual(
                self.results[(fwd, rev)].revGc,
                round(ResultsTest._getGc(rev), 1),
                f"{FAIL_MSG}{fwd}",
            )
    ...
    def testF_gcPercentInRange(self) -> None:
        ...
        for fwd, rev in self.results.keys():
            self.assertGreaterEqual(
                self.results[(fwd, rev)].fwdGc, self.params.minGc, f"{FAIL_MSG}{fwd}"
            )
            self.assertGreaterEqual(
                self.results[(fwd, rev)].revGc, self.params.minGc, f"{FAIL_MSG}{rev}"
            )
            self.assertLessEqual(
                self.results[(fwd, rev)].fwdGc, self.params.maxGc, f"{FAIL_MSG}{fwd}"
            )
            self.assertLessEqual(
                self.results[(fwd, rev)].revGc, self.params.maxGc, f"{FAIL_MSG}{rev}"
            )
    ...
    def testI_noHomoDimers(self) -> None:
        ...
        for fwd, rev in self.results.keys():
            self.assertTrue(
                ResultsTest._noHomoDimers(
                    fwd, self.params.minTm, self.params.tempTolerance
                ),
                f"{FAIL_MSG}{fwd}",
            )
            self.assertTrue(
                ResultsTest._noHomoDimers(
                    rev, self.params.minTm, self.params.tempTolerance
                ),
                f"{FAIL_MSG}{rev}",
            )
    ...
    def testL_ingroupHasOneProduct(self) -> None:
        ...
        for pair in self.results.keys():
            for name in ResultsTest.INGROUP_FILES:
                ...
                self.assertEqual(
                    len(self.results[pair].additional[name][ResultsTest.PCRLEN]),
                    1,
                    f"{FAIL_MSG_A}{pair}{FAIL_MSG_B}{name}",
                )
                self.assertEqual(
                    len(self.results[pair].additional[name][ResultsTest.CONTIG]),
                    1,
                    f"{FAIL_MSG_A}{pair}{FAIL_MSG_B}{name}",
                )
    ...
    def testM_ingroupProductsWithinRange(self) -> None:
        ...
        for pair in self.results.keys():
            for name in ResultsTest.INGROUP_FILES:
                ...
                self.assertGreaterEqual(
                    self.results[pair].additional[name][ResultsTest.PCRLEN][0],
                    self.params.minPcr,
                    f"{FAIL_MSG_A}{pair}{FAIL_MSG_B}{name}",
                )
                self.assertLessEqual(
    ...
    def testN_outgroupProductsSavedCorrectly(self) -> None:
        ...
        for pair in self.results.keys():
            for name in ResultsTest.OUTGROUP_FILES:
                ...
                numContigs = len(
                    self.results[pair].additional[name][ResultsTest.CONTIG]
                )
                numPcrLens = len(
                    self.results[pair].additional[name][ResultsTest.PCRLEN]
                )
                self.assertEqual(
                    numContigs, numPcrLens, f"{FAIL_MSG_A}{pair}{FAIL_MSG_B}{name}"
                )
        ...
        for pair in self.results.keys():
            for name in ResultsTest.OUTGROUP_FILES:
                ...
                for idx in range(
                    ...
                    if (
                        self.results[pair].additional[name][ResultsTest.CONTIG][idx]
                        == "NA"
                    ):
                        self.assertEqual(
                            self.results[pair].additional[name][ResultsTest.PCRLEN][
                            ...
                            0,
                            f"{FAIL_MSG_1A}{pair}{FAIL_MSG_B}{name}",
                        )

                    # if the pcr size is zero, then the contig should be NA
                    if (
                        self.results[pair].additional[name][ResultsTest.PCRLEN][idx]
                        == 0
                    ):
                        self.assertEqual(
                            self.results[pair].additional[name][ResultsTest.CONTIG][
                            ...
                            "NA",
                            f"{FAIL_MSG_2A}{pair}{FAIL_MSG_B}{name}",
                        )
        ...
        for fwd, rev in self.results.keys():
            ...
            for name in ResultsTest.INGROUP_FILES:
                ...
                for contig in fbind[name].keys():
                    fCount += len(fbind[name][contig][Primer.PLUS]) + len(
                        fbind[name][contig][Primer.MINUS]
                    )
                    rCount += len(rbind[name][contig][Primer.PLUS]) + len(
                        rbind[name][contig][Primer.MINUS]
                    )

                # each ingroup genome should have exactly one binding site for each primer
                self.assertEqual(
                    rCount, 1, f"{rev}{FAIL_MSG_A}{rCount}{FAIL_MSG_B}{name}"
                )
                self.assertEqual(
                    fCount, 1, f"{fwd}{FAIL_MSG_A}{fCount}{FAIL_MSG_B}{name}"
                )
                ...
                for contig in fbind[name].keys():
                    ...
                    if fbind[name][contig][Primer.PLUS] != []:
                        ...
                        self.assertNotEqual(
                            rbind[name][contig][Primer.MINUS],
                            [],
                            f"{rev} is not on opposite strand of {fwd} in {name}|{contig}",
                        )
                        ...
                        self.assertEqual(
                            fbind[name][contig][Primer.MINUS],
                            [],
                            f"{fwd} is on both strands in {name}|{contig}",
                        )

                        # the reverse primer should not be on the (+) strand
                        self.assertEqual(
                            rbind[name][contig][Primer.PLUS],
                            [],
                            f"{rev} is on the same strand as {fwd} in {name}|{contig}",
                        )
                    ...
                    # if the forward primer is on the (-) strand
                        ...
                        self.assertNotEqual(
                            rbind[name][contig][Primer.PLUS],
                            [],
                            f"{rev} is ont on opposite strand of {fwd} in {name}|{contig}",
                        )

                        # the forward primer should not be on the (+) strand
                        self.assertEqual(
                            fbind[name][contig][Primer.PLUS],
                            [],
                            f"{fwd} is on both strands in {name}|{contig}",
                        )
    ...
    def testS_isProductLengthCorrect(self) -> None:
        ...
        for fwd, rev in self.results.keys():
            ...
            for name in ResultsTest.INGROUP_FILES:
                ...
                pcrLen = self.results[(fwd, rev)].additional[name][ResultsTest.PCRLEN][
                    0
                ]
                contig = self.results[(fwd, rev)].additional[name][ResultsTest.CONTIG][
                    0
                ]
                ...
                self.assertEqual(
                    pcrLen, truLen, f"bad pcr product sizes in {name} for {fwd}, {rev}"
                )
            ...
            for name in ResultsTest.OUTGROUP_FILES:
                ...
                if contigs == ["NA"]:
                    self.assertEqual(
                        tru,
                        dict(),
                        f"'NA' in {name} but products produced by {fwd},{rev}",
                    )
                    ...
                    for contig in res.keys():
                        ...
                        self.assertEqual(
                            res[contig],
                            tru[contig],
                            f"unexpected sizes in {name} using {fwd}, {rev}",
                        )
                        ...
                        for plen in tru[contig]:
                            self.assertNotIn(
                                plen,
                                self.params.disallowedLens,
                                f"disallowed sizes in {name} with {fwd},{rev}",
                            )
    ...
    def testT_allPairsWritten(self) -> None:
        ...
        self.assertEqual(
            len(expected),
            len(observed),
            f"expected {len(expected)} primer pairs but observed {len(observed)}",
        )
        self.assertSetEqual(
            expected, observed, "expected pairs do not match those written to file"
        )
    ...
    def testU_arePairsSortedCorrectly(self) -> None:
        ...
        self.assertListEqual(
            expected,
            observed,
            "expected sort order does not match the order written to file",
        )
    ...
    def testV_primerTmsWereCorrect(self) -> None:
        ...
        for primer in primers:
            ...
            self.assertAlmostEqual(
                primer.hairpinTm, primer3.calc_hairpin_tm(str(primer)), places=3
            )
            self.assertAlmostEqual(
                primer.rcHairpin,
                primer3.calc_hairpin_tm(str(primer.reverseComplement())),
                places=3,
            )

            # check homodimer Tms
            self.assertAlmostEqual(
                primer.homodimerTm, primer3.calc_homodimer_tm(str(primer)), places=3
            )
            self.assertAlmostEqual(
                primer.rcHomodimer,
                primer3.calc_homodimer_tm(str(primer.reverseComplement())),
                places=3,
            )
    ...
    def testX_bedFileHasAllPrimers(self) -> None:
        ...
        self.assertSetEqual(
            observed, expected, "BED file primers do not match expected values"
        )
    ...
    def testY_bedFileCorrect(self) -> None:
        ...
        sortOrder = [
            (str(f.seq), str(r.seq)) for f, r in _sortPairs(self.params, self.pairs)
        ]
        ...
        for primer, num in self.bed.keys():
            ...
            self.assertIn(
                str(primer),
                sortOrder[num],
                f"unexpected sort order for {primer}: {num}",
            )
            ...
            for contig in self.bed[(primer, num)].keys():
                ...
                self.assertNotEqual(
                    self.bindingSites[primer][name][contig][strand],
                    [],
                    f"strand mismatch for {primer} on {name}: {contig}",
                )
                ...
                self.assertEqual(
                    ...
                    start + len(primer),
                    f"end position is not correct for {primer} on {name}: {contig}",
                )
                ...
                self.assertEqual(
                    start,
                    self.bindingSites[primer][name][contig][strand][0],
                    f"start positions for {name}: {contig} do not match for {primer}",
                )
