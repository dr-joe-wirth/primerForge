...
def __extractKmerPositions(
    seq: str, encodings: dict[int, set[int]]
    ...
    """
    # initialize output
    out = {Primer.PLUS: defaultdict(list), Primer.MINUS: defaultdict(list)}
...
def __productSizesFromStartPositions(
    plusStarts: list[int], minusStarts: list[int]
) -> set[int]:
...
def __getOutgroupProductSizes(
    kmers: dict[str, dict[str, list[int]]], fwd: str, rev: str
) -> set[int]:
    ...
    try:
        productSizes = __productSizesFromStartPositions(
            kmers[Primer.PLUS][fwd], kmers[Primer.MINUS][rev]
        )
    ...
    try:
        productSizes = __productSizesFromStartPositions(
            kmers[Primer.PLUS][rev], kmers[Primer.MINUS][fwd]
        )
        out.update(productSizes)
...
def __processOutgroupResults(
    outgroupProducts: dict[str, dict[tuple[Primer, Primer], set[tuple[str, int]]]],
    ...
    ingroupName = next(
        iter(
            [
                ...
                for n in v.keys()
                if n not in outgroupProducts.keys()
            ]
        )
    )
    ...
    for pair in pairs.keys():
        ...
        for name in outgroupProducts.keys():
            ...
            pairs[pair][name] = Product(
                contig,
                size,
                FAKE_BIN,
                FAKE_BIN,
                pairs[pair][ingroupName].dimerTm,
                FAKE_COORD,
                FAKE_COORD,
                FAKE_STRAND,
                FAKE_COORD,
...
def _removeOutgroupPrimers(
    pairs: dict[tuple[Primer, Primer], dict[str, Product]], params: Parameters
) -> None:
    ...
    for name in outgroupKmers.keys():
        ...
        if name != prevName:
            ...
            params.log.debug(
                f"{MSG_5A}{startNumPairs - len(pairs)}{MSG_5B}{prevName}{MSG_5C}{len(pairs)}{MSG_5D}"
            )
        ...
        for contig in outgroupKmers[name].keys():
            ...
            for fwd, rev in list(pairs.keys()):
                ...
                outgroupProducts[name][(fwd, rev)] = outgroupProducts[name].get(
                    (fwd, rev), set()
                )

                # get the outgroup products for this primer pair
                products = __getOutgroupProductSizes(
                    outgroupKmers[name][contig], str(fwd), str(rev)
                )
                ...
                # if there are products
                    ...
                    if not done:
                        outgroupProducts[name][(fwd, rev)].update(
                            {(contig, x) for x in products}
                        )

    # log the number of pairs removed and remaining from the last genome if debugging
    params.log.debug(
        f"{MSG_5A}{startNumPairs - len(pairs)}{MSG_5B}{name}{MSG_5C}{len(pairs)}{MSG_5D}"
    )
