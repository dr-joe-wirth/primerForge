...
def __binCandidateKmers(
    candidates: dict[str, list[Primer]], maxBinSize: int, isFirstGenome=False
) -> dict[str, dict[int, list[Primer]]]:
...
def __getBinPairs(
    binned: dict[str, dict[int, list[Primer]]],
    minPrimerLen: int,
    minProdLen: int,
    maxProdLen: int,
    ...
    for contig in binned.keys():
        ...
        sortedBins = sorted(
            binned[contig].keys(),
            key=lambda x: min(binned[contig][x][0].start, binned[contig][x][0].end),
        )
        ...
        for idx in range(len(sortedBins) - 1):
            ...
            for jdx in range(idx + 1, len(sortedBins)):
                ...
                smallest = (bin2[0].start + minPrimerLen) - (
                    bin1[-1].end - minPrimerLen
                )
...
def _formsDimers(
    ...
    """
    dimerTm = primer3.calc_heterodimer_tm(
        fwd,
        rev,
        mv_conc=mvConc,
        dv_conc=dvConc,
...
def __isPairSuitable(
    fwd: Primer, rev: Primer, minPcr: int, maxPcr: int, maxTmDiff: float
) -> tuple[bool, int]:
    ...
    if not minPcr <= pcrLen <= maxPcr:
        return False, pcrLen

    # unsuitable if the Tm difference is not within the allowed range
    if not abs(fwd.Tm - rev.Tm) <= maxTmDiff:
        return False, pcrLen

    return True, pcrLen


def __evaluateOnePair(
    fwd: str,
    rev: str,
    binPair: str,
    fTm: float,
    ...
    if not forms:
        return binPair, dimerTm


def __getCandidatePrimerPairs(
    binPairs: list[tuple[str, int, int]],
    bins: dict[str, dict[int, list[Primer]]],
    params: Parameters,
) -> list[tuple[Primer, Primer, Product]]:
                ...
                if hasGcClamp(fwd.seq[-5:]):
                    ...
                    for jdx, rev in enumerate(bin2):
                        ...
                        if hasGcClamp(rev.seq[:5]):
                            ...
                            suitable, pcrLen = __isPairSuitable(
                                fwd, rev, params.minPcr, params.maxPcr, params.maxTmDiff
                            )
...
def __restructureCandidateKmerData(
...
def __getAllSharedPrimerPairs(
    firstName: str,
    candidateKmers: dict[str, dict[str, list[Primer]]],
    candidatePairs: list[tuple[Primer, Primer, Product]],
    params: Parameters,
) -> dict[tuple[Primer, Primer], dict[str, Product]]:
...
def __updateBinsForUnprocessedGenomes(
    name: str,
    maxBinSize: int,
    kmers: dict[str, list[Primer]],
    pairs: dict[tuple[Primer, Primer], dict[str, Product]],
) -> None:
...
def _getPrimerPairs(
    candidateKmers: dict[str, dict[str, list[Primer]]], params: Parameters
) -> dict[tuple[Primer, Primer], dict[str, Product]]:
    ...
    binnedCandidateKmers = __binCandidateKmers(
        candidateKmers[firstName], params.maxBinSize, isFirstGenome=True
    )

    # get the bins that could work
    binPairs = __getBinPairs(
        binnedCandidateKmers, params.minLen, params.minPcr, params.maxPcr
    )
    ...
    for name in unprocessed:
        __updateBinsForUnprocessedGenomes(
            name, params.maxBinSize, candidateKmers[name], pairs
        )
